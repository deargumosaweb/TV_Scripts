...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
		<RBM_POST>
            <clip>
                <material_id><xsl:value-of select="translate(/MarquisEDL/ClipList/Clip/File, '_', '/')"/></material_id>
                <file_name><xsl:value-of select="/MarquisEDL/ClipList/Clip/File"/>.mxf</file_name>
                <title><xsl:value-of select="/MarquisEDL/ClipList/Clip/CustomMetadata/CustomData[@name='Title']"/></title>
                <material_type>Programme</material_type>
                <aspect_ratio>16:9</aspect_ratio>
                <video_format>HD</video_format>
                <content_log>
                    <file_start><xsl:value-of select="/MarquisEDL/ClipList/Clip/Start"/></file_start>
                    <programme_start>
					<xsl:call-template name="tc:add">
                            <xsl:with-param name="timecode1"><xsl:value-of select="/MarquisEDL/ClipList/Clip/Start"/></xsl:with-param>
                            <xsl:with-param name="timecode2">00:00:05:00</xsl:with-param>
                            <xsl:with-param name="fps1"><xsl:value-of select="/MarquisEDL/FrameRate"/></xsl:with-param>
                            <xsl:with-param name="fps2"><xsl:value-of select="/MarquisEDL/FrameRate"/></xsl:with-param>
                     </xsl:call-template>
                    </programme_start>
                    <programme_end><xsl:value-of select="/MarquisEDL/ClipList/Clip/End"/></programme_end>
                </content_log>
            </clip>
        </RBM_POST>
    </xsl:template>
